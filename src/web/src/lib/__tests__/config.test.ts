...
describe("config", () => {
  ...
  it("uses VITE_API_BASE_URL in SSR when window is undefined", async () => {
  ...
  it("falls back to production URL in SSR when window is undefined and VITE_API_BASE_URL is not set", async () => {
    vi.stubGlobal("window", undefined);
