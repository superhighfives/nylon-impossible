...
struct TaskCreationServiceTests {
    ...
    /// `FetchDescriptor`'s `SortDescriptor` uses a comparator that isn't a
    /// plain codepoint comparison — mixed-case fractional-index keys (as
    /// generated by `generateKeyBetween`) sort differently under it than
    /// under `<`. `fetchAllTodos` must sort with `<` to match the ordering
    /// every other position comparison in the app relies on.
    @Test("fetchAllTodos orders mixed-case fractional keys the same as <")
    @MainActor
    func fetchAllTodosOrdersMixedCaseKeysLikeRawComparison() throws {
        let container = try makeContainer()
        let context = container.mainContext

        let items = [
            TodoItem(title: "First", position: "A0000000000000000000000001"),
            TodoItem(title: "Second", position: "Zz"),
            TodoItem(title: "Third", position: "a0"),
            TodoItem(title: "Fourth", position: "b1V"),
            TodoItem(title: "Fifth", position: "z")
        ]
        for item in items {
            context.insert(item)
        }
        ...
        let todos = TaskCreationService.fetchAllTodos(userId: nil, context: context)
        let expected = items.sorted { $0.position < $1.position }.map(\.title)

        #expect(todos.map(\.title) == expected)
    }
